...
class Encoder(nn.Module):
    def __init__(self):
        ...
        # Define a new linear layer to expand vector_32d from 32 to 64 dimensions
        self.expand_vector_32d = nn.Linear(2, 128)  # Expands vector_32d from 32 to 64 dimensions
    ...
    def forward(self, x, vector_32d):
        ...
        out3d = self.conv2(out3d)
        out = self.pooling(res_out3d)
        out = out.view(b, -1)  # 展平为 (batch_size, feature_dim)
        z = self.fc(out)  # 编码器生成的特征向量 z，形状 (batch_size, 128)


        # Apply linear transformation to vector_32d to expand its dimensions
        vector_32d_expanded = self.expand_vector_32d(vector_32d)  # Now has shape (batch_size, 128)
        vector_repeated = vector_32d_expanded.unsqueeze(2).repeat(1, 1, 16)  # 重复 16 次，变为 [1, 128, 16]


        # # 拼接 z 和 vector_32d
        # z = torch.cat([z, vector_32d_expanded], dim=-1)  # 拼接后的形状为 (batch_size, 128 + 128 = 256)
        attention_output_z_to_v = self.crossA2V(src=out3d, tar=vector_repeated)  # z -> vector_32d_expanded
        attention_output_v_to_z = self.crossV2A(src=vector_repeated, tar=out3d)  # vector_32d_expanded -> z
        attention_output_z_to_v = attention_output_z_to_v.permute(1, 2, 0)
        attention_output_v_to_z = attention_output_v_to_z.permute(1, 2, 0)
        attention_output_z_to_v = self.pooling( attention_output_z_to_v)
        attention_output_v_to_z = self.pooling(attention_output_v_to_z)
        attention_output_z_to_v = attention_output_z_to_v.view(b, -1)  # 展平为 (batch_size, feature_dim)
        attention_output_v_to_z = attention_output_v_to_z.view(b, -1)  # 展平为 (batch_size, feature_dim)
        attention_output_z_to_v = 0.1*attention_output_z_to_v + z
        z1 = torch.cat([attention_output_z_to_v, attention_output_v_to_z],dim=-1)  # 形状为 (batch_size, 1024)
        return z1
...
class FilteredNoiseShaper(nn.Module):
    ...
    if __name__ == "__main__":
        ...
        noise_size = 16
        target_size = 48000
        vector_32d_size = 2  # vector_32d 的大小
        extra_param3_size = 1
